function preprocessBarChartGeneralStat(data, club_name) {
    result = []
    columns = ['Wins', 'Losses', 'Yellow Cards', 'Red Cards', 'Hit Woodwork', 'Goals', 'Goals Conceded', 'Clean Sheet']
    columns1 = ['wins', 'losses', 'total_yel_card', 'total_red_card', 'hit_woodwork', 'goals', 'goals_conceded', 'clean_sheet']
...
async function plotClubStatistic(fileName, club_name) {
    try {
        let test_data = await d3.csv(fileName);

        res = preprocessBarChartGeneralStat(test_data, club_name)

        max_races_won = Math.max(...res.map(o => o.Stats))
        ...
        var svg = d3.select("#club-statistics")
            .append("svg")
